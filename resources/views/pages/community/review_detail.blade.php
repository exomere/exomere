                    ...
                    <div class="max-w-2xl mx-auto p-4 bg-white">
                        ...
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-gray-700">{{ $item->author_name }}</span>
                            </div>
                        ...
                        <div class="flex items-center gap-1 mb-4">
                            @for ($i = 1; $i <= 5; $i++)
                                ...
                                @endif
                            @endfor
                            <span class="ml-1 text-lg">{{ $item->rating }}</span>
                        </div>
                        ...
                        <!-- Review image -->
                        <div class="mb-6">
                            @if(isset($item->file1) && is_file($item->file1))
                                <img src="{{ asset($item->file1) }}" alt="Product review"
                                     class="rounded-lg w-full h-auto object-cover"
                                     onerror="this.src='//exomere.co.kr/storage/data/noimg.jpg';"/>
                            @endif
                            @if(isset($item->file2) && is_file($item->file2))
                                <img src="{{ asset($item->file2) }}" alt="Product review"
                                     class="rounded-lg w-full h-auto object-cover"
                                     onerror="this.src='//exomere.co.kr/storage/data/noimg.jpg';"/>
                            @endif
                            @if(isset($item->file3) && is_file($item->file3))
                                <img src="{{ asset($item->file3) }}" alt="Product review"
                                     class="rounded-lg w-full h-auto object-cover"
                                     onerror="this.src='//exomere.co.kr/storage/data/noimg.jpg';"/>
                            @endif
                            @if(isset($item->file4) && is_file($item->file4))
                                <img src="{{ asset($item->file4) }}" alt="Product review"
                                     class="rounded-lg w-full h-auto object-cover"
                                     onerror="this.src='//exomere.co.kr/storage/data/noimg.jpg';"/>
                            @endif
                            @if(isset($item->file5) && is_file($item->file5))
                                <img src="{{ asset($item->file5) }}" alt="Product review"
                                     class="rounded-lg w-full h-auto object-cover"
                                     onerror="this.src='//exomere.co.kr/storage/data/noimg.jpg';"/>
                            @endif
                        </div>
                            ...
                            <div class="flex items-center gap-4">
                                <button class="flex items-center gap-1 like-button"
                                        ...
                                        @if(request()->session()->get('member_seq'))
                                            onclick="ajaxLikeReview({{ $item->id }})"
                                        @else
                                    ...
                                    <span>
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                                             class="liked size-4 {{ $item->liked ? '' : 'hidden' }}">
                                          <path
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                             ...
                                             class="unliked size-4 {{ $item->liked ? 'hidden' : '' }}">
                                    ...
                                    <span>
                                        ...
                                        <span class="helpful_count">{{ $item->likes_count ?? 0 }}</span>
                                    </span>
                                </button>
                            </div>
...
@section('page-script')
    <script>
        ...
        function ajaxLikeReview(reviewId) {
            ...
            const elem = document.querySelector('.helpful_count');
            ...
            fetch(`/ajax/reviews/${reviewId}/like`, {
                ...
                .then(data => {
                    if (data.liked) {
                        ...
                        document.querySelector('.unliked').classList.add('hidden');

                        elem.innerText = count + 1;
                    } else {
                        document.querySelector('.liked').classList.add('hidden');
                        document.querySelector('.unliked').classList.remove('hidden');
